...
def main(myWindow, args):
    ...
    if args.resume:
        checkpoint = torch.load(args.resume, map_location=myWindow.device, weights_only=True)
    ...
    metrics = {
        'train_losses': train_list[0],  # 모든 에포크의 훈련 손실 리스트
        'train_accuracies': train_list[1],  # 모든 에포크의 훈련 정확도 리스트
        'val_losses': valid_list[0],  # 모든 에포크의 검증 손실 리스트
        'val_accuracies': valid_list[1]  # 모든 에포크의 검증 정확도 리스트
    }
