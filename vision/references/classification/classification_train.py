...
from classification_transforms import get_mixup_cutmix
from classification_project import *
import numpy as np
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, rec_train, model_ema=None, scaler=None):
    model.train()
    metric_logger = classification_utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter("lr", classification_utils.SmoothedValue(window_size=1, fmt="{value}"))
    metric_logger.add_meter("img/s", classification_utils.SmoothedValue(window_size=10, fmt="{value}"))
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        acc1, acc5 = classification_utils.accuracy(output, target, topk=(1, 2)) #5 -> 2(클래스 개수 dog, cat 2개로 수정.)
...
def evaluate(model, criterion, data_loader, device, rec_valid, print_freq=100, log_suffix="", epoch=0):
    ...
    with torch.inference_mode():
        for image, target in metric_logger.log_every(data_loader, print_freq, header):
            ...
            acc1, acc5 = classification_utils.accuracy(output, target, topk=(1, 2))
    ...
    num_processed_samples = classification_utils.reduce_across_processes(num_processed_samples)
    if (
...
def load_data(traindir, valdir, args):
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        dataset = torchvision.datasets.ImageFolder(
            traindir,
            classification_presets.ClassificationPresetTrain(
        ...
        if args.cache_dataset:
            ...
            classification_utils.mkdir(os.path.dirname(cache_path))
            classification_utils.save_on_master((dataset, traindir), cache_path)
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        if args.weights and args.test_only:
            ...
            preprocessing = classification_presets.ClassificationPresetEval(
        ...
        if args.cache_dataset:
            ...
            classification_utils.mkdir(os.path.dirname(cache_path))
            classification_utils.save_on_master((dataset_test, valdir), cache_path)
...
def main(myWindow,args):
    ...
    parameters = classification_utils.set_weight_decay(
        model,
        args.weight_decay,
    ...
    if args.model_ema:
        ...
        model_ema = classification_utils.ExponentialMovingAverage(model_without_ddp, device=device, decay=1.0 - alpha)
