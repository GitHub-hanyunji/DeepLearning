...
class WindowClass(QTabWidget, form_class):
    def __init__(self):
        ...
        self.out_directory.clicked.connect(lambda:self.openFolderDialog('output_path'))
         # 옵션 세팅 저장하는 버튼
        self.setting_save.clicked.connect(lambda:self.run_command('setting'))

        # 모델학습 버튼 클릭시 run_command 메서드 호출
        self.model_teach.clicked.connect(lambda:self.run_command('train'))
    ...
    def run_command(self,message):
        ...
        if(message=="train"):
            if(self.resultfolder_path!="" and self.folder_path!=""):
                args = detection_train.get_args_parser(self.folder_path,epoch,aspect,dataset,model,weight,self.device,self.resultfolder_path)
    ...
    def test(self):
        if(self.device!=""):    
            ...
            model = torch.load(f'{self.model_path}/model.pth',map_location=self.device)
            model = model.to(self.device)  # 모델을 선택한 장치로 이동
            model.eval()
                ...
                for class_name, predictions in best_predictions.items():
                    for best_score, best_box in predictions:
                        ...
                        cv2.putText(img_cv, f'{class_name}: {best_score:.4f}', 
                                    (int(best_box[0][0]), int(best_box[0][1])), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), thickness=6)
            ...
            self.file_dirShow2.setText("device를 선택하지 않았습니다.")    
